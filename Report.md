...
本次实验我使用了三种启发式函数$h_1, h_2, h_3$。

- $h_1$使用曼哈顿距离
- $h_2$使用**Euclid**距离
- $h_3$使用切比雪夫距离

不难证明，$h_1, h_2, h_3$均是可采纳的，即：$h_1,h_2,h_3\leq h^\star$。

但是 $h_1,h_2,h_3$ 不满足单调性，证明如下：

考虑一种简单情形，$0, 8$已经正确归位，现在将$0,8$互换，$h$增大$2$，但是两种状态的转移代价为$1$。

由于 $h_1, h_2,h_3$ 不满足单调性，所以不能够进行环检测。
...
如图可见，无论是A* 还是IDA* ，$h_1,h_2$的效果都十分接近，在某些数据点$h_1$更优秀，某些数据点$h_2$更优秀。而$h_3$的效果明显不如二者。

这与常识相符，理论上，对于所有满足可采纳性的$h\leq h^\star$，$h$ 越大，越接近$h^\star$，效果应该越好。

原因很简单，A* 算法拓展的节点集合为$S = \lbrace x:h(x)\leq h(ed)\rbrace$，而$h(ed) = h^\star(ed)$，所以$h$越大，$|S|$越小，算法的效率越高。当$h = h^\star$时，算法只会沿着最优路径拓展，此时效率最高。

在这里，显然$h_3\leq h_2\leq h_1$，所以$h_3$的效果最差，但是，$h_1$应该比$h_2$更优秀才对，不知道为什么两者效果如此接近。
